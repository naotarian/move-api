...
class RegionPrefectureService
{
     ...
     * @return array|null ['region' => string, 'prefecture' => string] または null
     */
    ...
    /**
     * 都道府県名から地域と都道府県のモデルオブジェクトを取得
     *
     * @param string $prefectureName 都道府県名
     * @return array|null ['region' => Region, 'prefecture' => Prefecture] または null
     */
    public static function getModelsByPrefectureName(string $prefectureName): ?array
    {
        $prefecture = Prefecture::with('region')->byName($prefectureName)->first();
...
/**
 * 住所から地域と都道府県を取得するヘルパー関数（後方互換性のため）
 *
 * @param string $address 住所文字列
 * @return array|null ['region' => string, 'prefecture' => string] または null
 */
function getRegionAndPrefecture(string $address): ?array
{
    return \App\Services\Common\RegionPrefectureService::getRegionAndPrefecture($address);
}
